...
function fillTrie(csvArray){
    ...
    for (var i=0; i < csvArray.length ; i++){
        ...
        trie.insert(BinaryRoute,csvArray[i][0]);
...
function create_tuple_rules(){
    ...
    for (var i = 1; i < traffic_report.length; i++) {
        ...
        srcNet = srcNet == undefined ? src : srcNet.value
        dstNet = trie.longest_prefix_match(cidr2bin(dst))
        dstNet = dstNet == undefined ? dst : dstNet.value

        Net2tuplematch = `${dstNet},${port}`  

        if (newRules.has(Net2tuplematch)) {
            newRules.get(Net2tuplematch).add(srcNet)
        }
        else {
            const sourceSet = new Set();
            sourceSet.add(srcNet)
            newRules.set(Net2tuplematch, sourceSet)
        }
    }
    ...
    for (const [Net2tuple, sources] of newRules.entries()) {
        index += 1 
        dstport = Net2tuple.split(",")
        dst = dstport[0]
        port = dstport[1]

        joinedsources = Array.from(sources).join(",")


        template = `set rulebase security rules "Allow flow ${index}" to any
set rulebase security rules "Allow flow ${index}" from any
set rulebase security rules "Allow flow ${index}" source [ ${joinedsources} ]
set rulebase security rules "Allow flow ${index}" destination [ ${dst} ]
set rulebase security rules "Allow flow ${index}" source-user any
set rulebase security rules "Allow flow ${index}" category any
set rulebase security rules "Allow flow ${index}" application any
set rulebase security rules "Allow flow ${index}" service any
set rulebase security rules "Allow flow ${index}" source-hip any
set rulebase security rules "Allow flow ${index}" destination-hip any
set rulebase security rules "Allow flow ${index}" action allow
set rulebase security rules "Allow flow ${index}" log-setting default`
